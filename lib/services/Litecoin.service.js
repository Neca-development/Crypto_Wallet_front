...
class litecoinService {
    constructor() {
        this.network = coininfo_1.default.litecoin.main.toBitcoinJS();
    }
    ...
    async getTokensByAddress(address) {
        ...
        let ltcToUSD;
        try {
            ltcToUSD = (await axios_1.default.get(`${providers_1.backendApi}coins/LTC`, {
                headers: {
        ...
        tokens.push(this.generateTokenObject(nativeTokensBalance, 'LTC', providers_1.imagesURL + 'LTC.svg', 'native', ltcToUSD.data.usd));
        return tokens;
    }
    async getFeePriceOracle(from, to, amount, tokenTypes) {
        amount = Math.trunc(amount * 1e8);
        const sochain_network = 'LTC', sourceAddress = from, utxos = await axios_1.default.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
        ...
        utxos.data.data.txs.forEach(async (element) => {
            fee = (inputCount * 146 + outputCount * 33 + 10) * providers_1.litecoinSatoshisPerByte;
        ...
        const ltcToUSD = (await axios_1.default.get(`${providers_1.backendApi}coins/LTC`, {
            headers: {
                'auth-client-key': providers_1.backendApiKey,
            },
        })).data;
    ...
    async getTransactionsHistoryByAddress(address, pageNumber, pageSize) {
        const { data: ltcToUSD } = await axios_1.default.get(`${providers_1.backendApi}coins/LTC`, {
            headers: {
                'auth-client-key': providers_1.backendApiKey,
            },
        });
    ...
    async sendMainToken(data) {
        const sochain_network = 'LTC', privateKey = data.privateKey, sourceAddress = this.keys.publicKey, utxos = await axios_1.default.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`), 
        // @ts-ignore
