...
export class litecoinService {
    ...
    generateKeyPair(mnemonic) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('%cMyProject%cline:44%cprivateKey', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(161, 23, 21);padding:3px;border-radius:2px', privateKey1);
            const publicKey2 = addrFromMnemonic.toHDPrivateKey().derive("m/44'/1'/0'/0/0").privateKey.toAddress('testnet').toString();
            console.log('%cMyProject%cline:45%cpublicKey', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(222, 125, 44);padding:3px;border-radius:2px', publicKey2);
            // -----------------------------------------------------------------------------------------------------------------
            ...
            console.log('%cMyProject%cline:45%caddress', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(179, 214, 110);padding:3px;border-radius:2px', address.toString());
            const litecoinXprv = {
            ...
            console.log('%cMyProject%cline:69%cbip32RootKey', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(248, 214, 110);padding:3px;border-radius:2px', bip32RootKey.toBase58());
            const keyPair = bitcoin.ECPair.fromWIF(bip32RootKey.toWIF(), litecoinXprv);
            console.log('%cMyProject%cline:78%ckeyPair', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(254, 67, 101);padding:3px;border-radius:2px', keyPair.privateKey);
    ...
    getTransactionsHistoryByAddress(address) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { data: btcToUSD } = yield axios.get(`${backendApi}coins/BTC`, {
                headers: {
                    'auth-client-key': backendApiKey,
                },
            });
    ...
    sendMainToken(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sochain_network = 'LTCTEST', privateKey = data.privateKey, sourceAddress = this.keys.publicKey, utxos = yield axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`), transaction = new bitcoin.TransactionBuilder(litecoinXprv), amount = Math.trunc(data.amount * 1e8), privateKeyECpair = bitcoin.ECPair.fromPrivateKey(Buffer.from(privateKey, 'hex'), { network: litecoinXprv });
            console.log('%cMyProject%cline:231%cprivateKeyECpair', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(1, 77, 103);padding:3px;border-radius:2px', privateKeyECpair);
