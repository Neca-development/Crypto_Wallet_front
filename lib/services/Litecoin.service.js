...
export class litecoinService {
    ...
    async getTokensByAddress(address) {
        ...
        let ltcToUSD;
        try {
            ltcToUSD = (await axios.get(`${backendApi}coins/LTC`, {
                headers: {
    ...
    async getFeePriceOracle(from, to, amount, tokenTypes) {
        ...
        const sochain_network = 'LTC', sourceAddress = from, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
        ...
        const ltcToUSD = (await axios.get(`${backendApi}coins/LTC`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
        })).data;
    ...
    async getTransactionsHistoryByAddress(address, page_number, page_size) {
        const { data: ltcToUSD } = await axios.get(`${backendApi}coins/LTC`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
        });
    ...
    async sendMainToken(data) {
        const sochain_network = 'LTC', privateKey = data.privateKey, sourceAddress = this.keys.publicKey, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`), 
        // @ts-ignore
