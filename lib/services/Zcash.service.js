...
export class zcashService {
    ...
    sendMainToken(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('%cMyProject%cline:281%cprivateKeyECpair', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(153, 80, 84);padding:3px;border-radius:2px', privateKeyECpair);
            let totalInputsBalance = 0, fee = 0, inputCount = 0, outputCount = 2;
            console.log(utxos);
            utxos.data.data.txs.sort((a, b) => {
                if (Number(a.value) > Number(b.value)) {
