...
export class zcashService {
    ...
    async getTokensByAddress(address) {
        ...
        let zcashToUSD;
        try {
            zcashToUSD = (await axios.get(`${backendApi}coins/ZEC`, {
                headers: {
    ...
    async getFeePriceOracle(from, to, amount, tokenTypes) {
        ...
        const sochain_network = 'ZEC', sourceAddress = from, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
        ...
        const { data: zcashToUSD } = await axios.get(`${backendApi}coins/ZEC`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
        });
    ...
    async sendMainToken(data) {
        ...
        // @ts-ignore
        transaction = new utxolib.bitgo.ZcashTransactionBuilder(netGain), utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
