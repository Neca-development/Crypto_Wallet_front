...
export class solanaService {
    ...
    getTokensByAddress(address) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // USDT Balance
            const info = yield this.connection.getTokenAccountsByOwner(new PublicKey(address), { mint: new PublicKey(solanaUSDTContractAddress) });
            let USDTTokenBalance;
            if (info.value[0]) {
                const tokenBalance = yield this.connection.getTokenAccountBalance(new solanaWeb3.PublicKey(info.value[0].pubkey));
                USDTTokenBalance = tokenBalance.value.uiAmount;
            }
            else {
                USDTTokenBalance = 0;
            }
            tokens.push(this.generateTokenObject(USDTTokenBalance, 'Tether USDT', imagesURL + 'USDT.svg', 'custom', solToUSD.data.usd, solToUSD.data.usdt, solanaUSDTContractAddress));
            return tokens;
        });
    }
    ...
    send20Token(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const fromTokenAccount = yield mintToken.getOrCreateAssociatedAccountInfo(this.address.publicKey);
            const receiverAddress = new PublicKey(data.receiverAddress);
            const toTokenAccount = yield mintToken.getOrCreateAssociatedAccountInfo(receiverAddress);
            const transaction = new solanaWeb3.Transaction()
                .add(Token.createTransferInstruction(TOKEN_PROGRAM_ID, fromTokenAccount.address, toTokenAccount.address, this.address.publicKey, [], data.amount * 100));
            const signature = yield solanaWeb3.sendAndConfirmTransaction(this.connection, transaction, [this.address], { commitment: 'confirmed' });
            return signature;
        });
    }
