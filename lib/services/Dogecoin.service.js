...
export class dogecoinService {
    ...
    generateKeyPair(mnemonic) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('%cMyProject%cline:30%cdogecore.HDPrivateKey.fromSeed(seed).deriveChild(m/44/3/0/0/0).privateKey', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(20, 68, 106);padding:3px;border-radius:2px', dogecore.HDPrivateKey.fromSeed(seed).deriveChild("m/44'/1'/0'/0/0").privateKey);
    ...
    sendMainToken(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('%cMyProject%cline:189%ccoininfo.dogecoin', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(178, 190, 126);padding:3px;border-radius:2px', coininfo.dogecoin.test);
            var frmt = curr.toBitcoinJS();
            console.log('%cMyProject%cline:197%cfrmt', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(38, 157, 128);padding:3px;border-radius:2px', frmt);
            const netGain = {
            ...
            const sochain_network = 'DOGETEST', privateKey = data.privateKey, sourceAddress = this.keys.publicKey, utxos = yield axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`), transaction = new bitcoin.TransactionBuilder(netGain), amount = Math.trunc(data.amount * 1e8), privateKeyECpair = bitcoin.ECPair.fromPrivateKey(Buffer.from(privateKey, 'hex'), { network: netGain });
