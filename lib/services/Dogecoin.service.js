...
export class dogecoinService {
    constructor() {
        this.network = coininfo.dogecoin.main.toBitcoinJS();
    }
    ...
    async getTokensByAddress(address) {
        ...
        let dogeToUSD;
        try {
            dogeToUSD = (await axios.get(`${backendApi}coins/DOGE`, {
                headers: {
    ...
    async getFeePriceOracle(from, to, amount, tokenTypes) {
        ...
        const sochain_network = 'DOGE', sourceAddress = from, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
        ...
        const dogeToUSD = (await axios.get(`${backendApi}coins/DOGE`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
        })).data;
    ...
    async getTransactionsHistoryByAddress(address, page_number, page_size) {
        const { data: dogeToUSD } = await axios.get(`${backendApi}coins/DOGE`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
        });
    ...
    async sendMainToken(data) {
        const sochain_network = 'DOGE', privateKey = data.privateKey, sourceAddress = this.keys.publicKey, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`), 
        // @ts-ignore
