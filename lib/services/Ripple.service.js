...
class rippleService {
    constructor() {
        this.xrplClient = new xrpl.Client(providers_1.rippleProvider);
    ...
    async getTokensByAddress(address) {
        ...
        let xrplToUSD;
        try {
            xrplToUSD = (await axios_1.default.get(`${providers_1.backendApi}coins/XRP`, {
                headers: {
        ...
        tokens.push(this.generateTokenObject(balance, 'XRP', providers_1.imagesURL + 'XRP.svg', 'native', xrplToUSD.data.usd));
        return tokens;
    }
    async getFeePriceOracle(from, to, amount, tokenTypes) {
        ...
        const { data: xrplToUSD } = await axios_1.default.get(`${providers_1.backendApi}coins/XRP`, {
            headers: {
                'auth-client-key': providers_1.backendApiKey,
            },
        });
    ...
    convertTransactionToCommonFormat(txData, tokenPriceToUSD, address) {
        ...
        const tokenLogo = providers_1.imagesURL + 'XRP.svg';
