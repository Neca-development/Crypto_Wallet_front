...
export class bitcoinService {
    ...
    async getTokensByAddress(address) {
        ...
        let btcToUSD;
        try {
            btcToUSD = (await axios.get(`${backendApi}coins/BTC`, {
                headers: {
    ...
    async getFeePriceOracle(from, to, amount, tokenTypes, speed) {
        ...
        const sochain_network = 'BTCTEST', sourceAddress = from, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
    ...
    async getTransactionsHistoryByAddress(address, page_number, page_size) {
        ...
        const { data: btcToUSD } = await axios.get(`${backendApi}coins/BTC`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
        });
