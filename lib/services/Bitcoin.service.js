...
export class bitcoinService {
    ...
    getFeePriceOracle(from, to, amount) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const sochain_network = 'BTCTEST', sourceAddress = from, utxos = yield axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
            ...
            const btcToUSD = (yield axios.get(`${backendApi}coins/BTC`, {
                headers: {
                    'auth-client-key': backendApiKey,
                },
            })).data;
            const usd = Math.trunc(Number(btcToUSD.data.usd) * value * 100) / 100;
            console.log('%cMyProject%cline:130%cvar', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(60, 79, 57);padding:3px;border-radius:2px', { value, usd });
            return {
                value,
                usd,
            };
