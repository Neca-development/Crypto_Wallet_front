...
export class dashService {
    constructor() {
        this.network = coininfo.dash.main.toBitcoinJS();
    }
    ...
    async getTokensByAddress(address) {
        ...
        let dashToUSD;
        try {
            dashToUSD = (await axios.get(`${backendApi}coins/DASH`, {
                headers: {
    ...
    async getFeePriceOracle(from, to, amount, tokenTypes) {
        ...
        const sochain_network = 'DASH', sourceAddress = from, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
        ...
        const { data: dashToUSD } = await axios.get(`${backendApi}coins/DASH`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
        });
    ...
    async getTransactionsHistoryByAddress(address, page_number, page_size) {
        const { data: dashToUSD } = await axios.get(`${backendApi}coins/DASH`, {
            headers: {
                'auth-client-key': backendApiKey,
            },
    ...
    async sendMainToken(data) {
        const sochain_network = 'DASH', privateKey = data.privateKey, sourceAddress = this.keys.publicKey, utxos = await axios.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`), 
        // @ts-ignore
        ...
        const { data: trRequest } = await axios.post(`${backendApi}transactions/so-chain/${sochain_network}`, {
