...
class dashService {
    constructor() {
        this.network = coininfo_1.default.dash.main.toBitcoinJS();
    }
    async generateKeyPair(mnemonic) {
        const seed = (0, bip39_1.mnemonicToSeedSync)(mnemonic);
        const root = bitcoin.bip32.fromSeed(seed, this.network).derivePath("m/44'/5'/0'/0/0");
    ...
    async getTokensByAddress(address) {
        ...
        let dashToUSD;
        try {
            dashToUSD = (await axios_1.default.get(`${providers_1.backendApi}coins/DASH`, {
                headers: {
        ...
        tokens.push(this.generateTokenObject(nativeTokensBalance, 'DASH', providers_1.imagesURL + 'DASH.svg', 'native', dashToUSD.data.usd));
        return tokens;
    }
    async getFeePriceOracle(from, to, amount, tokenTypes) {
        amount = Math.trunc(amount * 1e8);
        const sochain_network = 'DASH', sourceAddress = from, utxos = await axios_1.default.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`);
        ...
        utxos.data.data.txs.forEach(async (element) => {
            fee = (inputCount * 146 + outputCount * 33 + 10) * providers_1.dashSatoshisPerByte;
        ...
        const { data: dashToUSD } = await axios_1.default.get(`${providers_1.backendApi}coins/DASH`, {
            headers: {
                'auth-client-key': providers_1.backendApiKey,
            },
        });
    ...
    async sendMainToken(data) {
        const sochain_network = 'DASH', privateKey = data.privateKey, sourceAddress = this.keys.publicKey, utxos = await axios_1.default.get(`https://sochain.com/api/v2/get_tx_unspent/${sochain_network}/${sourceAddress}`), 
        // @ts-ignore
        transaction = new bitcoin.TransactionBuilder(this.network), amount = Math.trunc(data.amount * 1e8), privateKeyECpair = bitcoin.ECPair.fromWIF(privateKey, this.network);
        let totalInputsBalance = 0, fee = 0, inputCount = 0, outputCount = 2;
