...
export class binanceService {
    ...
    getTokensByAddress(address) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            tokens.push(this.generateTokenObject(Number(this.web3.utils.fromWei(nativeTokensBalance)), 'BNB', 'native', bnbToUSD.binancecoin.usd));
            tokens.push(this.generateTokenObject(USDTTokenBalance, 'Tether USDT', 'custom', bnbToUSD.binancecoin.usd, binanceUSDTContractAddress));
            return tokens;
        });
    }
    ...
    generateTokenObject(balance, tokenName, tokenType, trxToUSD, contractAddress) {
        const tokenPriceInUSD = tokenType === 'custom' ? 1 : trxToUSD;
        const balanceInUSD = tokenType === 'custom' ? Math.trunc(balance * 100) / 100 : Math.trunc(balance * trxToUSD * 100) / 100;
        return {
            balance,
            balanceInUSD,
            contractAddress,
