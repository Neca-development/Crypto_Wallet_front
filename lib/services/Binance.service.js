...
export class binanceService {
    ...
    getTransactionsHistoryByAddress(address) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const transactions = [];
            const trxTransactions = yield this.getNormalTransactions(address, Number(bnbToUSD.data.usd));
    ...
    convertTransactionToCommonFormat(txData, address, bnbToUSD) {
        const tokenLogo = 'http://135.181.216.90:49352/api/images/USDT.svg', to = txData.to, from = txData.from, amount = this.web3.utils.fromWei(txData.value), fee = +(+this.web3.utils.fromWei((txData.gasUsed * txData.gasPrice).toString())).toFixed(6), direction = from === address ? 'OUT' : 'IN', amountInUSD = (Math.trunc(+amount * bnbToUSD * 100) / 100).toString();
        return {
            to,
            from,
            amount,
