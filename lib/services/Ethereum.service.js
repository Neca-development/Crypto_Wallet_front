...
import { etherScanApi, etherScanApiKey, ethWeb3Provider, etherUSDTContractAddress, etherGasPrice, backendApi, } from '../constants/providers';
...
export class ethereumService {
    ...
    getTransactionsHistoryByAddress(address) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const transactions = [];
            const trxTransactions = yield this.getNormalTransactions(address, Number(ethToUSD.data.usd));
    ...
    convertTransactionToCommonFormat(txData, address, ethToUSD) {
        const tokenLogo = 'http://135.181.216.90:49352/api/images/USDT.svg', to = txData.to, from = txData.from, amount = this.web3.utils.fromWei(txData.value), fee = +(+this.web3.utils.fromWei((txData.gasUsed * txData.gasPrice).toString())).toFixed(6), direction = from === address ? 'OUT' : 'IN', amountInUSD = (Math.trunc(+amount * ethToUSD * 100) / 100).toString();
        return {
            to,
            from,
            amount,
