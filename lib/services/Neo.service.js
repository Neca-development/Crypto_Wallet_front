...
export class neoService {
    ...
    getTokensByAddress(address) {
        return __awaiter(this, void 0, void 0, function* () {
            const balance = yield api.getTokenBalances(address);
            console.log('%cMyProject%cline:70%cbalance', 'color:#fff;background:#ee6f57;padding:3px;border-radius:2px', 'color:#fff;background:#1f3c88;padding:3px;border-radius:2px', 'color:#fff;background:rgb(39, 72, 98);padding:3px;border-radius:2px', balance);
            const tokens = [];
            ...
            tokens.push(this.generateTokenObject(balance.assets['NEO'].balance.toNumber, 'NEO', imagesURL + 'NEO.svg', 'native', neoToUSD.data.usd, neoToUSD.data.usdt, etherUSDTContractAddress));
            return tokens;
        });
    }
