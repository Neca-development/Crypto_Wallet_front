...
export class neoService {
    ...
    getTokensByAddress(address) {
        return __awaiter(this, void 0, void 0, function* () {
            let balance = 0;
            const balanceResponse = yield this.rpcClient.execute(new rpc.Query({
                method: 'getnep17balances',
                params: [address],
            }));
            for (const token of balanceResponse.balance) {
                const { assethash, amount } = token;
                const tokenNameResponse = yield this.rpcClient.invokeFunction(assethash, 'symbol').catch((e) => {
                    console.error({ e });
                });
                //@ts-ignore
            ...
            const tokens = [];
            const { data: neoToUSD } = yield axios.get(`${backendApi}coins/NEO`, {
                headers: {
