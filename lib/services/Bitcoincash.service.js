...
// @ts-ignore
const bitboxSdk = __importStar(require("bitbox-sdk"));
...
class bitcoincashService {
    ...
    async getTokensByAddress(address) {
        ...
        let bchToUSD;
        try {
            bchToUSD = (await axios_1.default.get(`${providers_1.backendApi}coins/BCH`, {
                headers: {
        ...
        tokens.push(this.generateTokenObject(addressInfo.balance, 'BCH', providers_1.imagesURL + 'BCH.svg', 'native', bchToUSD.data.usd));
        return tokens;
    }
    async getFeePriceOracle(from, to, amount) {
        ...
        u.utxos.forEach(async (utxo) => {
            fee = Math.floor(this.bitbox.BitcoinCash.getByteCount({ P2PKH: inputCount }, { P2PKH: 2 }) * providers_1.bitcoincashSatoshisPerByte);
    ...
    async getTransactionsHistoryByAddress(address, pageNumber, pageSize) {
        ...
        const { data: bchToUSD } = await axios_1.default.get(`${providers_1.backendApi}coins/BCH`, {
            headers: {
                'auth-client-key': providers_1.backendApiKey,
            },
        });
